      ...
      switch(type)
      {
        ...
        case EVILIB_TELE_V:
	  			if(speed >= EVILIB_min_zspeed && speed <= EVILIB_max_zspeed)
	    			sprintf(buffer, "8%d0104072%XFF", Id_cam, speed);
	  			else
          {
        ...
        case EVILIB_WIDE_V:
	  			if(speed >= EVILIB_min_zspeed && speed <= EVILIB_max_zspeed)
	    			sprintf(buffer, "8%d0104073%XFF", Id_cam, speed);
	  			else
        	{
	      		pthread_mutex_unlock(&BufferDispo);
	      		return 0;
 ...
 * the 'zoom' value has different range acording to the Combined/Separated mode. Be carefull !!
 * waitC: wait for completion of command: EVILIB_NO_WAIT_COMP
 *                                        EVILIB_WAIT_COMP
 * Return 1 on success, 0 on error
 */
  ...
  if(power == EVILIB_ON)
	...
	case EVILIB_SEPARATE_MODE:
	  ...
	  break;
        case EVILIB_TELE_V:
	  if(speed >= EVILIB_min_zspeed && speed <= EVILIB_max_zspeed)
	    sprintf(buffer, "8%d0104062%XFF", Id_cam, speed);
	  else
            {
	      pthread_mutex_unlock(&BufferDispo);
	      return 0;
	  ...
	  break;
        case EVILIB_WIDE_V:
	  if(speed >= EVILIB_min_zspeed && speed <= EVILIB_max_zspeed)
	    sprintf(buffer, "8%d0104063%XFF", Id_cam, speed);
	  else
            {
	      pthread_mutex_unlock(&BufferDispo);
	      return 0;
