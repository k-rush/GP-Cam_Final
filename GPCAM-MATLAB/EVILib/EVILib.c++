...
void *Receiver(void *arg)
{
  ...
  do
  {
    ...
    while((char)c != '\xff' && commonData->End != EVILIB_OFF)
    {
      ...
      {
	      case -1:
					//cout << "Reveiver() : case -1\n";
		      commonData->answer = 0;
		      break;
        case 0:
	      	no_signal_reicv ++;
					if (commonData->waitComp == EVILIB_WAIT_COMP)
					{
	      		if(no_signal_reicv >= 100000)//all commands should take no more than 10 seconds!
          	{
		  				mutex_return = pthread_mutex_trylock(&commonData->TakeComp);
		  				if(mutex_return == 0)
		    				pthread_mutex_unlock(&commonData->TakeComp);
		  				else if((mutex_return == EBUSY && commonData->waitComp == EVILIB_WAIT_COMP))
		      		{
								commonData->answer = 0;
...
//    _port = open(portname, O_RDWR | O_NOCTTY | O_NONBLOCK | O_NDELAY);
  _port = open(portname, O_CREAT | O_RDWR| O_NOCTTY, 0666 );
...
int EVILib::convert(int type, float value, float limit, float maxi)
{
  ...
  switch(type)
	...
	case 0: 
	case 1: x = 0; break;
	case 2: x = 0x1606; break;
	case 3: x = 0x2151; break;
	case 4: x = 0x2860; break;
	case 5: x = 0x2CB5; break;
	case 6: x = 0x3060; break;
	case 7: x = 0x32D3; break;
	case 8: x = 0x3545; break;
	case 9: x = 0x3727; break;
	case 10: x = 0x38A9; break;
	case 11: x = 0x3A42; break;
	case 12: x = 0x3B4B; break;
	case 13: x = 0x3C85; break;
	case 14: x = 0x3D75; break;
	case 15: x = 0x3E4E; break;
	case 16: x = 0x3EF7; break;
	case 17: x = 0x3FA0; break;
	case 18: x = 0x4000; break;
	default: x = 0; break;
      }
      if(x < 0.0)
				x = 0.0;
...
float EVILib::deconvert(float limit, float maxi, int start, int length)
{
  ...
  c = 1;
	...
	//Constantin
	/*printf("EVILIB::deconvert() - Current buffer3: ");
	for(i = 0; i < 50; i++)
		printf("%X", (unsigned char)buffer3[i]);
	printf("\n");*/
...
//  loop only once.
      ...
      pthread_mutex_lock(&TakeComp);
//-----------------------------------------
	//sleep(10);	//wait for the camera to power on!
	...
	if(power == EVILIB_ON)
  {
    ...
    switch(type)
      ...
      case EVILIB_RESET:
	  		...
	  		break;
      default:
	  		if(pan_speed >= min_pspeed && pan_speed <= max_pspeed && tilt_speed >= min_tspeed && tilt_speed <= max_tspeed)
        {
		
	      	switch(type)
 					{
          	case EVILIB_ABSOLUTE:
		  				sprintf(buffer, "8%d010602%.2X%.2X", Id_cam, pan_speed, tilt_speed);
            ...
            case EVILIB_RELATIVE:
		  				sprintf(buffer, "8%d010603%.2X%.2X", Id_cam, pan_speed, tilt_speed);
		 					make0XString(convert(EVILIB_PAN, pan_pos, maxpan, maxPan), &buffer[strlen(buffer)], 4);
		  				make0XString(convert(EVILIB_TILT, tilt_pos, maxtilt, maxTilt), &buffer[strlen(buffer)], 4);
		  				strcat(buffer, "FF");
		  				break;
            default:
		  				sprintf(buffer, "8%d010601%.2X%.2X", Id_cam, pan_speed, tilt_speed);
		  				switch(type)
              {
              	case EVILIB_UP:
    ...
    {//failed to send
			pthread_mutex_unlock(&BufferDispo);
			cout << "Pan_TiltDrive() : failed to send command.\n";
	  	return 0;
    }
    pthread_mutex_unlock(&BufferDispo);
// receiv 'ACK'
